...
function Goat( name, image ) {
  ...
  this.votes = 0;
  this.views = 0;
  this.speak = function() { console.log('Baaaaah!');}
  state.goats.push( this );
}

// Load from local storage --or-- create goats
function loadGoats() {
  // Read the key "goats" from LS
  let savedGoats = localStorage.getItem('goats');

  // If we have them, re-instantiate the objects
  if( savedGoats ) {
    let parsedGoats = JSON.parse(savedGoats);
    for( let goat of parsedGoats ) {
      let newGoat = new Goat( goat.name, goat.image );
      newGoat.votes = goat.votes;
      newGoat.views = goat.views;
    }
  }
  // If we don't have them, create them from scratch
  else {
   createGoats();
  }
  console.log('state.goats:', state.goats);
}
...
// Save the goats to local storage
function saveGoats() {
  let stringifiedGoats = JSON.stringify(state.goats);
...
// Do we have goat in local storge?
